...
const appPagesDir = join(repoRoot, "site2", "app", "pages");
const legacyPagesDir = join(repoRoot, "site", "pages");
...
const pages = (await findHtmlFiles(legacyPagesDir)).map((source) => {
  const legacyRoute = relative(join(repoRoot, "site"), source).replaceAll("\\", "/");
...
for (const page of pages) {
  ...
  const routeDir = join(repoRoot, "site2", "app", page.legacyRoute.replace(/\.html$/, ""));
  await mkdir(routeDir, { recursive: true });

  const componentImport = importPath(routeDir, join(repoRoot, "site2", "components", "LegacyPage.tsx"));
  const dataImport = importPath(routeDir, join(repoRoot, "site2", "data", "legacy-pages.ts"));
